...
```{r bad-plot2, echo=FALSE, fig.align="center", out.width="90%", fig.cap="Finally a pie chart that makes sense."}
...
```{r comparison-plot, fig.cap='Price of differently colored diamonds. Red circles are means, black circles are individual data poins, and the error bars are 95% bootstrapped confidence intervals.'}
...
3. I've used `shape = 21` for displaying the mean. For this circle shape, we can set a `color` and `fill` (see Figure \@ref(fig:shapes))

```{r shapes, fig.cap="Different shapes that can be used for plotting."}
...
```{r boxplots, fig.cap="Box plot distributions. Source: https://www.autodeskresearch.com/publications/samestats"}
include_graphics("figures/boxplots.gif")
```

Figure \@ref(fig:boxplots) shows three different distributions that each correspond to the same boxplot. 
...
```{r boxplot-violin, fig.cap="Boxplot distributions. Source: https://www.autodeskresearch.com/publications/samestats"}
...
```{r}
# install.packages("ggpol") # run this once if you don't have the package yet
...
```{r}
# install.packages("ggridges") # run this once if you don't have the package yet
...
- For the data argument in `ggplot()` use: `df.diamonds[1000, ]` (this selects the first 1000 rows).
...
- Figure \@ref(fig:comparison-plot) will help you with figuring out the other components

```{r}
# make the plot here 
```

```{r practice-plot1, echo=F, fig.cap='Practice plot 1.'}
...
Why does this plot look different from the one before? What went wrong here? Notice that the data frame associated with the ggplot2 object has `r nrow(info)` rows. So instead of plotting means here, we plotted all the individual data points. So what we are seeing here is just the top layer of many, many layers. 
...
```{r code-snippets, fig.cap="Enable code snippets."}
