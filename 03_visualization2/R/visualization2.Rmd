...
```{r, eval=FALSE}
install.packages(c("gganimate", "gapminder", "ggridges", 
                   "devtools", "png", "gifski", "patchwork"))
```
...
ggplot(data = df.diamonds, 
  ...
  theme(
    ...
    plot.subtitle = element_text(margin = margin(b = 0.3,
                                                 unit = "inch"),
                                 color = "gray70"),  
...
Here, I'm saving the plot in the `figures` folder and it's name is `diamond_plot.pdf`. I also specify the width and height of the plot in inches (which is the default unit). 
...
If a plot looks weird, it's worth taking a look behind the scenes. For example, something we think we could have tried is the following (in fact, this is what I tried first): 
...
Check out how effective the animations are in this talk from the famous Dr. Hans Rosling 
- [best stats you've ever seen](https://www.youtube.com/watch?v=hVimVzgtD6w)
...
Often, we want to create similar plots over and over again. One way to achieve this is by finding the original plot, copy and pasting it, and changing the bits that need changing. Another more flexible and faster way to do this is by using snippets. Snippets are another type of short-cut for typing out short pieces of code. 
