...
df.power = df.power %>% # generate random data
  ...
  mutate(fit = map(.x = data, 
                   .f = ~ binom.test(x = .$response, # define formula
                                     n = .$n,
                                     p = 0.5,
                                     alternative = "two.sided")))
df.power
...
# run simulation 
df.power2 = expand_grid(n = seq(from = 10, to = 60, by = 2),
                        simulation = 1:n_simulations) %>%
  mutate(index = 1:n()) %>% 
  group_by(index, n, simulation) %>% 
  mutate(data = list(tibble(group1 = rnorm(n = n,
                            ...
                            group2 = rnorm(n = n,
                                           ...
                                           sd = sd2)) %>% 
                       pivot_longer(cols = everything()))) %>% 
  mutate(p.value = map(.x = data,
                       .f = ~ t.test(formula = value ~ name,
                                     data = .x)$p.value)) %>%
  group_by(n) %>% 
  summarize(power = sum(p.value < 0.05) / n()) %>% 
  ungroup()
