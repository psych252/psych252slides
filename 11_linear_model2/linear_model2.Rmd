...
fit2 = df.credit %>% 
  ...
  # mutate_at(vars(income, limit, education), funs(scale)) %>% 
  lm(balance ~ 1 + income + limit + education, data = .)
...
fit_2 = lm(resid ~ newspaper, df.fit1)

fit_2 %>% summary()


fit_1 = lm(sales ~ newspaper, df.ads)
...
fit_c = lm(formula = balance ~ 1, data = df.credit)
fit_a = lm(formula = incoem + balance ~ student + income:student, data = df.credit)
...
df.credit %>% 
  mutate(student_dummy = ifelse(student == "No", 0, 1)) %>% 
  mutate(income_student = income * student_dummy) %>% 
  select(balance, income, student, student_dummy, income_student)
...
fit1 = lm(formula = balance ~ income + student + income_student, data = df.credit)
fit2 = lm(formula = balance ~ income + student + income:student, data = df.credit)
