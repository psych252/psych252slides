...
```{r modeling-data-01, message=FALSE}
...
```{r modeling-data-03, warning=F, fig.cap="Tradeoff between fit and model simplicity."}
...
```{r modeling-data-04, fig.cap="Figure that I used to illustrate that fitting more data points with fewer parameter is more impressive."}
# make example reproducible 
set.seed(1)
# n_samples = 20
n_samples = 3
...
```{r modeling-data-07, warning=FALSE, fig.cap="Sum of absolute errors."}
value_predicted = 7
...
```{r modeling-data-09, echo=F, fig.cap="Relationship between error definition and best estimators."}
...
```{r modeling-data-10, fig.cap="Mean, median, and mode on the normal distribution."}
...
```{r modeling-data-11, fig.cap="Mean, median, and mode on the exponential distribution."}
rate = 1 
...
```{r modeling-data-14, warning=F}
df.internet = read_table2(file = "data/internet_access.txt") %>% 
  clean_names()
```

```{r modeling-data-15, fig.cap="Selection of the data."}
df.internet %>% 
  mutate(i = 1:n()) %>% 
...
```{r modeling-data-16, fig.cap="Sum of squared errors, proportional reduction in error (PRE), F statistic, p-value."}
...
```{r modeling-data-17, fig.cap="F-distribution"}
df1 = 1
df2 = 49
...
```{r modeling-data-20, warning=F, message=F}
...
df.samples = samples %>% 
  as_tibble(.name_repair = ~ 1:ncol(samples)) %>% 
  mutate(sample = 1:n()) %>% 
  pivot_longer(cols = -sample,
               names_to = "index",
               values_to = "value") %>% 
...
```{r modeling-data-21, warning=F, message=F}
samples %>% 
