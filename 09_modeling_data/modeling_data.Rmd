...
# run this code chunk once to install these packages
...
dnorm(point, mean = param_mean, sd = param_sd)
...
df.model %>% 
  summarize(augmented_sse = sum(augmented_se),
            ...
            pvalue = 1-pf(f, pa-pc, nrow(df.model)-1),
            mean = mean(internet),
            sd = sd(internet)) %>% 
              kable() %>% 
              kable_styling(bootstrap_options = "striped",
...
m_c = lm(internet~-1+75, data = df.internet)
m_a = lm(internet~1, data = df.internet)


# anova(m_a, )


tmp = t.test(df.internet$internet, mu = 75)
tmp %>% 
  tidy()
...
Simple model with internet access fixed to 75.
