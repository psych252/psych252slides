...
Make some adjustments to the Tools > Global Options
...
ggplot(data = diamonds, mapping = aes(x = cut, y = price)) +
  ...
  stat_summary(fun.data = "mean_cl_boot", geom = "linerange", linewidth = 1.5) +
...
There are three main aspects of data the form the building blocks of much of statistics:   
Means (or more generally, _central tendency_)
Variance (or more generally, _dispersion_)
Correlation (or more generally, _covariance_)
So, when plotting data, it is often useful to consider whether and how one is visually representing these core building blocks. 

We start with _central tendency_.
...
We often don't just want to show the means (_central tendency_) but also give a sense for how much the data varies (_dispersion_). `ggplot2` has some convenient ways of specifying error bars. Let's take a look at how much `price` varies as a function of `clarity` (another variable in our `diamonds` data frame).
...
# I've changed the order in which the layers for the means and error bars are drawn.
ggplot(df.diamonds,
       aes(x = clarity,
           y = price)) +
  stat_summary(fun = "mean",
...
Let's take a look at two more principles for plotting data that are extremely helpful: groups and facets. But before that, another practice plot. 
