...
```{r anscombe, echo=FALSE, fig.cap="Anscombe's quartet."}
...
```{r correlations, echo=FALSE, fig.cap= "The Pearson's $r$ correlation coefficient is the same for all of these datasets. Source: [Data Visualization -- A practical introduction by Kieran Healy](http://socviz.co/lookatdata.html#lookatdata)"}
include_graphics("figures/correlations.png")
```

All the datasets in Figure \@ref(fig:correlations) share the same correlation coefficient. However, again, they are very different from each other.

```{r datasaurus, echo=FALSE, fig.cap="__The Datasaurus Dozen__. While different in appearance, each dataset has the same summary statistics to two decimal places (mean, standard deviation, and Pearson's correlation)."}
...
[Here](https://www.autodeskresearch.com/publications/samestats) is the paper from which I took Figure \@ref(fig:anscombe) and \@ref(fig:datasaurus). It explains how the figures were generated and shows more examples for how summary statistics and some kinds of plots are insufficient to get a good sense for what's going on in the data.

```{r, echo=FALSE, fig.cap="Animation showing different data sets that all share the same summary statistics."}
...
```{r bad-plot1, echo=FALSE, out.width='90%', fig.align='center', fig.cap="Example of a bad plot. Source: [Data Visualization -- A practical introduction by Kieran Healy](http://socviz.co/lookatdata.html#lookatdata)"}
  include_graphics("figures/bad_plot1.png")
```

```{r bad-plot2, echo=FALSE, out.width='90%', fig.align='center', fig.cap="Another bad plot. Source: Google image search for 'bad graphs'"}
  include_graphics("figures/bad_plot2.png")
```

```{r bad-plot3, echo=FALSE, out.width='90%', fig.align='center', fig.cap="And another one. Source: [Bad graph wall of shame](http://bcuchta.com/wall_of_shame/)"}
  include_graphics("figures/bad_plot3.png")
```

```{r bad-plot4, echo=FALSE, out.width='90%', fig.align='center', fig.cap="And another one. Source: [Bad graph wall of shame](http://bcuchta.com/wall_of_shame/)"}
  include_graphics("figures/bad_plot4.png")
```

```{r bad-plot5, echo=FALSE, out.width='90%', fig.align='center', fig.cap="And another one. Source: [Bad graph wall of shame](http://bcuchta.com/wall_of_shame/)"}
  include_graphics("figures/bad_plot5.png")
```

```{r bad-plot6, echo=FALSE, out.width='90%', fig.align='center', fig.cap="The last one for now. Source: [Bad graph wall of shame](http://bcuchta.com/wall_of_shame/)"}
...
```{r pref-general, out.width='100%', echo=FALSE, fig.cap="General preferences."}
...
```{r pref-code, out.width='100%', echo=FALSE, fig.cap="Code window preferences."}
...
```{r scatterplot, fig.cap="Scatterplot."}
...
```{r scatterplot-color, fig.cap="Scatterplot with color."}
ggplot(data = df.diamonds,
       mapping = aes(x = carat,
                     y = price,
                     color = color)) +
...
Aha! Now we've got some color. Notice how in Figure \@ref(fig:scatterplot-color) `ggplot2` added a legend for us, thanks! We'll see later how to play around with legends. Form just eye-balling the plot, it looks like the diamonds with the best `color` (D) tended to have a lower `carat` value, and the ones with the worst `color` (J), tended to have the highest carat values.
...
```{r errorbars, fig.cap="Relationship between diamond clarity and price. Error bars indicate 95% bootstrapped confidence intervals."}
ggplot(data = df.diamonds,
...
Note how the error bars are larger now compared to Figure \@ref(fig:errorbars)
...
```{r order1, fig.cap='This figure looks good. Error bars and means are drawn in the correct order.'}
ggplot(df.diamonds, aes(x = clarity, y = price)) +
  stat_summary(fun.data = "mean_cl_boot", geom = "linerange") +
  stat_summary(fun.y = "mean", geom = "point", color = "red")
```

Figure \@ref(fig:order1) looks good.

```{r order2, fig.cap='This figure looks good. Error bars and means are drawn in the correct order.'}
...
Figure \@ref(fig:order2) doesn't look good. The error bars are on top of the points that represent the means.
...
ggplot(data = df.diamonds,
  ...
  scale_fill_manual(values = c("lightblue", "blue", "lightred", "red", "black"))
```
...
Figure \@ref(fig:stretching-it) is stretching it in terms of how much information it presents. But it gives you a sense for how to combine the differnet bits and pieces we've learned so far.
