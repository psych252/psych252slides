...
```{r datasaurus, echo=FALSE, fig.cap="__The Datasaurus Dozen__. While different in appearance, each dataset has the same summary statistics (mean, standard deviation, and Pearson's correlation) to two decimal places."}
...
Here, we specified that we want to plot color on the x-axis, and the average price on the y-axis. As you can see, `ggplot2` has already figured out how to label the axes. However, we still need to specify _how_ to plot it. Let's make a __bar graph__: 
...
Now, let's practice making some plots! 
...
- [styler](https://github.com/r-lib/styler) --> RStudio add-in that re-formats code using a good style 
