...
```{r model-comparison-01, include=FALSE, eval=FALSE}
install.packages("cowplot")
```

## Load packages and set plotting theme  

```{r model-comparison-02, message=FALSE}
...
Note the somewhat weird construction `~ mean(.$data$x[.$idx]))`. This is just because the `bootstrap` function stores the information about each boostrapped data set in that way. Each bootstrapped sample simply points to the original data set, and then uses a different set of indices `idx` to indicate which values from the original data set it sampled (with replacement). 
...
fit %>% 
  ...
  mutate(likelihood = dnorm(.resid, sd = sigma)) %>% 
  clean_names() %>% 
  select(x, y, fitted, resid, likelihood) %>% 
  head(10) %>% 
  kable(digits = 2) %>% 
  kable_styling(full_width = F)
```
