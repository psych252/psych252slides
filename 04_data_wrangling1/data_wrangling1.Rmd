...
```{r pipe, out.width = "90%", echo=FALSE, fig.cap="Inspiration for the `magrittr` package name."}
...
<!-- TODO: maybe remove the following two sectionss -->
...
`filter()` lets us apply logical operators (see Table \@ref(tab:operators)) to subset the data. Here, I've filtered out the male characters. 
...
We can combine multiple conditions in the same call. Here, I've filtered out male characters, whose height is greater than the median height (i.e. in the top 50 percentile), and whose mass was not `NA`. 
